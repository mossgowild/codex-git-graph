...
点击提交查看变更文件，方向键切换文件及 diff。关闭详情后保留选中提交并恢复焦点；刷新不会重新展开已关闭的详情。刷新后仍在加载范围内的提交保留所选对比父节点和文件。搜索框支持 Enter 跳转和 Shift+Enter 反向跳转。

选中一个提交后，按住 ⌘（Windows/Linux 为 Ctrl）点击另一个提交，或点击“比较…”再选择目标，比较两个提交。先选的是基准，后选的是目标，标题始终显示“基准 → 目标”；可点击“交换”反向比较、“结束比较”返回目标提交与其父节点的差异。键盘可用“比较…”、方向键和 Enter 完成选择。刷新保留比较范围和文件。

文件差异由 Monaco Diff Editor 显示，从 Git 对象读取两侧完整历史内容，不读取当前工作区内容来代替历史版本。新增和删除文件以空的一侧表示，重命名使用各自版本的路径，文件模式变化及子模块提交也会显示。文本按 UTF-8 读取，单侧超过 2 MiB、二进制或非 UTF-8 文件会显示说明，不截断后继续比较。仅换行符或 BOM 不同会明确提示。切换“行内／并排”调整视图，“展开”可用整个面板浏览详情；差异区域内 ⌘/Ctrl+F 搜索文件。编辑器为只读，配色和字体跟随宿主，代码、字体和 Worker 均随插件打包，无需访问 CDN。

同名引用在筛选框、时间线和提交详情中增加“本地 / 远程 / 标签”标识。分支标签按完整名称宽度显示，不使用固定宽度截断；提交列空间不足时，可拖宽该列查看完整名称。历史底部的“当前检出分支”是工作区状态，不随提交选择改变；提交详情中的“指向此提交”显示直接指向所选提交的分支与标签。没有直接引用时明确提示，不将工作区分支当作历史提交的所属分支。

切换筛选期间暂停旧列表和搜索交互；读取失败会恢复原筛选，重试仍针对请求的筛选。所选分支或标签被删除后，重新加载全部历史并明确提示。分页使用固定提交快照；若发现分支、标签或 HEAD 变化，会重新加载第一页并提示，避免混用旧历史和新引用。
...
浏览器回归检查使用 `npm run test:ui`，需要可用的 Playwright 和 Chromium。可通过 `PLAYWRIGHT_MODULE` 指向现有 Playwright 的模块入口，通过 `PLAYWRIGHT_CHROMIUM` 指向现有浏览器可执行文件；未设置时使用本地 Playwright 及其默认浏览器。检查会在临时数据目录中覆盖各列首次拖动的边界位移、取消、布局恢复和错误重试，不修改个人布局。另使用临时 Git 仓库覆盖筛选失败与重试、删除筛选引用后的恢复、分页期间引用变化、刷新时父节点与文件选择保留、引用重名、diff 内容着色及乱序响应。

在本机已安装当前清单版本后，运行 `npm run test:installed` 可额外检查实际缓存中的启动程序，确认它保留任务仓库及非 Git 目录；该检查不会安装插件或修改 Codex 配置。

图形布局和 Git 数据查询为插件自身实现；交互参考 VS Code Git Graph，未复制其受限许可源码。历史 diff 使用 MIT 许可的 Monaco Editor。DOMPurify 固定至 3.4.15，以避免 Monaco 0.56.0 默认依赖版本的已知漏洞。依赖许可见 [THIRD-PARTY-LICENSES.txt](plugins/git-graph/THIRD-PARTY-LICENSES.txt)。
