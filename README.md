...
在 Codex Desktop 的任务侧面板中，交互式浏览当前 Git 仓库。当前版本：0.2.0。
...
拖动列标题右侧的分隔线调整宽度；双击恢复该列默认宽度。分隔线支持 Tab 聚焦、左右键调整 8 px、Shift + 左右键微调 1 px，Home 恢复默认，Escape 取消拖动。窄面板保留所有列并允许横向滚动。关系图始终保留容纳全部分支线的最小宽度。
